...
admin_id = ****  # Замените на ваш реальный user_id
...
def text_to_speech(text, lang='ru'):
    filename = "samurai_granny_speech.mp3"
    try:
...
# Функция для общения с нейросетью OpenAI с вредным ответом от бабки самурая
def ask_openai(text):
    try:
        chat_completion = openai.ChatCompletion.create(
            model=model_name,
            messages=[
                {"role": "system", "content": "Ты старая бабка самурая, которая отвечает на вопросы с мудростью и строгостью, но с ворчливым тоном. Отвечай на вопросы, помогай людям, но не забывай о своей суровости."},
...
def handle_start(message):
    response_text = (
        "Привет, внучок! Я старая бабка самурая, и я буду учить тебя мудрости и дисциплине. "
        "Отправь мне сообщение, и я его повторю. "
        "Используй команду /granny, чтобы задать мне вопрос. "
...
def handle_help(message):
    help_text = (
        "Вот что я могу делать, внучок:\n"
        "/start - Приветственное сообщение от бабки самурая.\n"
        "/help - Показать это сообщение.\n"
        "/granny <текст> - Задать вопрос старой бабке самурая.\n"
...
# Обработка команды /granny
@bot.message_handler(commands=['granny'])
def handle_granny(message):
    text = message.text.split(' ', 1)[-1].strip()
    if text:
        ...
        bot.reply_to(message, "Ошибка: не указан текст для запроса к бабке самурая.")
